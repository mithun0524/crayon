...
async function detectScripts(
  ...
  try {
    ...
    const s = pkg.scripts ?? {};
    return {
      build: s.build ? `pnpm build` : undefined,
      test: s.test ? `pnpm test` : undefined,
      dev: s.dev ? `pnpm dev` : undefined,
      lint: s.lint ? `pnpm lint` : undefined,
    };
  } catch {
    return {};
  }
      ...
      execute: async ({ output_path }: { output_path?: string }) => {
        try {
          ...
          const absPath = path.isAbsolute(relPath)
            ? relPath
            : path.join(ctx.workspaceRoot, relPath);
